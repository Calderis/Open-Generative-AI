...
function getApiKey(request) {
    ...
    // Priority 2: replicate_key cookie (used by the fixed builder library)
    const cookieKey = request.cookies.get('replicate_key')?.value;
    return cookieKey;
}
...
export async function POST(request, { params }) {
    ...
    const apiKey = getApiKey(request);
    console.log(`[proxy POST] ${targetUrl} | apiKey: ${apiKey ? apiKey.slice(0,8)+'...' : 'MISSING'} | cookie: ${request.cookies.get('replicate_key')?.value?.slice(0,8) || 'NONE'} | header: ${request.headers.get('x-api-key')?.slice(0,8) || 'NONE'}`);
    if (apiKey) headers.set('x-api-key', apiKey);
