...
export function VideoStudio() {
    ...
    videoFileInput.onchange = async (e) => {
        ...
        showVideoSpinner();
        try {
            const url = await replicate.uploadFile(file);
    ...
    (async () => {
        ...
        pending.forEach(async (job) => {
            ...
            try {
                const result = await replicate.pollForResult(job.requestId, apiKey, attemptsLeft, job.interval);
    ...
    generateBtn.onclick = async () => {
        ...
        try {
            if (v2vMode) {
                const res = await replicate.processV2V({ model: selectedModel, video_url: uploadedVideoUrl, onRequestId });
            ...
            if (imageMode) {
                ...
                const res = await replicate.generateI2V(i2vParams);
            ...
            const res = await replicate.generateVideo(params);
