...
const STORAGE_KEY = 'replicate_key';
...
export default function StandaloneShell() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      document.cookie = `replicate_key=${stored}; path=/; max-age=31536000; SameSite=Lax`;
    }
  }, [fetchBalance]);
  ...
  const handleKeySave = useCallback((key) => {
    ...
    setApiKey(key);
    fetchBalance(key);
    document.cookie = `replicate_key=${key}; path=/; max-age=31536000; SameSite=Lax`;
  }, [fetchBalance]);
  ...
  const handleKeyChange = useCallback(() => {
    ...
    setApiKey(null);
    setBalance(null);
    document.cookie = "replicate_key=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
  }, []);
